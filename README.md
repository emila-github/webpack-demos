...
After launching the server, `index.js` will be minified into following.
...
On the surface, you won't feel any differences. However, Webpack actually builds `index.js` and `a.js` into different chunks(`bundle.js` and `1.bundle.js`), and loads `1.bundle.js` from `bundle.js` when on demand.
...
Now Webpack will build `index.js` into `bundle.js`, and `a.js` into `1.bundle.js`.
